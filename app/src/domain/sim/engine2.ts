import { mulberry32, shuffle, type SimDeck } from './rng';
import { model, type Effect } from './model';

/**
 * engine2 — Kampf-/Keyword-/Interaktions-Modell für Deck-Tests. Einheiten kommen
 * aufs Feld (mit Lag), greifen die Gig-Area an, der Verteidiger kann blocken;
 * Kämpfe entscheiden sich an der Power, besiegte Einheiten wandern in den Trash.
 * Keywords aus dem Kartentext (Lag/Adrenaline/Blocker/Go Solo) und imperative
 * Karteneffekte (Removal, Spend-Rival, Gig-Swing, Buff, Draw). Eddies kommen aus
 * den 3 Legends (+ Ramp), nicht aus einer abstrakten Kurve.
 *
 * Weiterhin ein MODELL, kein regeltreuer Simulator (Einzeltexte, Gear, Reaktionen,
 * Würfel-Details bleiben abstrahiert) — aber Interaktion zählt echt. Rein &
 * deterministisch (Seed) → reproduzierbar.
 */
